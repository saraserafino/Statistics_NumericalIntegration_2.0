...
// Constructor
StatOp::StatOp(const std::string &input_targetColumn) : targetColumn(input_targetColumn) {};

std::vector<std::optional<std::variant<double, std::string>>> StatOp::readSpecificColumn(const std::shared_ptr<CSVHandler> CSVfile, const std::string &targetColumn) {
...
std::variant<double, std::string> StatOp::calculateMedian(const std::vector<std::optional<std::variant<double, std::string>>> &data) {
...
double StatOp::calculateStandardDeviation(const std::vector<std::optional<std::variant<double, std::string>>> &data) {
...
std::string StatOp::calculateFrequency(const std::vector<std::optional<std::variant<double, std::string>>> &data) {
    ...
    for (const auto &entry : frequencyMap) {
        ...
        if (entry.first && entry.first.value().index() == 0) {
            ...
            result << std::setprecision(std::numeric_limits<double>::digits10) << std::get<double>(entry.first.value()) << ": " << entry.second << " times\n";
        } else if (entry.first && entry.first.value().index() == 1) {
            // If the variant holds a string, append to the result
            result << std::get<std::string>(entry.first.value()) << ": " << entry.second << " times\n";
        }
    }
...
std::string StatOp::calculateClassification(const std::shared_ptr<CSVHandler> CSVfile, const std::string &targetColumn, const std::string &condition) {
    // Get the name of the input file
...
double StatOp::calculateCorrelation(const std::vector<std::optional<std::variant<double, std::string>>> &data1, const std::vector<std::optional<std::variant<double, std::string>>> &data2) {
    ...
    for (size_t i = 0; i < data1.size(); i++) {
        if(data1[i] && data2[i]){
            ...
            numerator += (std::get<double>(data1[i].value()) - mean1) * (std::get<double>(data2[i].value()) - mean2);
            } else {
            // If the index of value is a string, throw an exception
